...
class GraphicsObject {
 public:
  ...
  /*
      ...
      It tells the compiler to generate the body for this function here
     only(thereby completing the definition.)
  */

  /*
      "=0" is a declaration. It tells the compiler "there is no defintion/body
     here", and derived classes must provide one. virtual ... = 0; are called
     pure virtual functions this means their derived must implement it(or it
     would be a compile error) virtual ...; may have their base class defn. and
     if derived classes don't override it. then base defn. maybe called. Like:
     QMouseEvent, QPaintEvent, etc.
  */
...
struct style {
  ...
  style(std::string s) {
    ...
    pos = s.find("stroke=\"") + 8;
    stroke =
        QColor(QString::fromStdString(s.substr(pos, s.find("\"", pos) - pos)));
