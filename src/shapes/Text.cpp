...
std::shared_ptr<GraphicsObject> Text::loadShape(const std::string& s1, const std::string& s2)
{
    auto style_ = style(s1);
    auto text = std::make_shared<Text>(style_.stroke, style_.fill, style_.strokeWidth);
    ...
    std::cout << "Bounding Box: (" << x << ", " << y << ") to (" << x + width << ", " << y + height << ")" << std::endl;
