...
std::shared_ptr<GraphicsObject> RoundRect::loadShape(const std::string& s1, const std::string& s2)
{
    auto style_ = style(s1);
    auto rect = std::make_shared<RoundRect>(style_.stroke,style_.fill , style_.strokeWidth);
    ...
    std::cout << "Loaded RoundRect with x: " << x << ", y: " << y << ", width: " << width << ", height: " << height << ", radius: " << radius_ << std::endl;
    return rect;
}
