...
void Diagram::destroy(std::shared_ptr<GraphicsObject>& obj) {
  ...
  // remove pushes the objects matching obj to the end of vector list and return
  // an iterator, then erase deletes all the elements from that iterator till
  // the end.
}

int Diagram::numOfShapes() const { return objs.size(); }
// void Diagram::undo_push_(const std::pair<std::shared_ptr<GraphicsObject>,
// int>& p)
// {
...
// void Diagram::redo_push_(const std::pair<std::shared_ptr<GraphicsObject>,
// int>& p)
// {
...
//     redo_push_(p);
//     if(p.second == 0) objs.erase(std::remove(objs.begin(), objs.end(),
//     p.first), objs.end()); else if(p.second == 1) objs.push_back(p.first);
// }
...
//     else if(p.second==1) objs.erase(std::remove(objs.begin(), objs.end(),
//     p.first), objs.end());
// }
